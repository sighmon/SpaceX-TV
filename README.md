...
- X API timeline discovery with a user-supplied [Bearer Token](https://docs.x.com/x-api/introduction), falling back to a hosted daily cache when no token is configured.
...
Discovery prefers X API v2 with the user's own Bearer Token:
...
When no Bearer Token is configured, the app falls back to `https://www.sighmon.com/spacex-tv/x-cache.json`. The fallback JSON is generated by `scripts/update_spacex_x_cache.rb` and contains the same X API response shapes used by the app's normal discovery path.
